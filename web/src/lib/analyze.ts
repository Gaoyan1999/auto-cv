...
function jdRequirementLines(jd: string): string[] {
  ...
  const scored = lines.map((l) => {
    let s = 0;
    if (
      /must|should|years|experience|required|strong|proficien|familiar/i.test(l)
    ) {
      s += 2;
    }
    if (/\d/.test(l)) {
...
function buildSuggestions(
  ...
  for (const b of bullets) {
    ...
    const term =
      missing[i % Math.max(missing.length, 1)] ??
      jdTop[i % Math.max(jdTop.length, 1)] ??
      '';
...
function rewriteBulletLine(
  ...
  const extra =
    jdTop.find((t) => !body.toLowerCase().includes(t)) ?? primaryTerm;
...
export function analyzeResumeAgainstJd(
  ...
  if (!resume.trim() || !jd.trim()) {
    return {
      result: {
        ...
        gaps: [
          'Paste a full posting and ensure your resume has content before running analysis.',
        ],
        traceRows: [],
      },
      suggestions: [],
  ...
  const traceRows = (
    reqLines.length
      ? reqLines
      : [jd.split(/\n/).find((l) => l.trim().length > 20) ?? jd.slice(0, 120)]
  ).map((line) => ({
